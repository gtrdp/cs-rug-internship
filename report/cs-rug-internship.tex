...
% ieee 802.11
WiFi communication in mobile phone seems 
...
A study also concerns about the multi-hop network effect on IEEE 802.11b wireless communication~\cite{Andriani2013}. By conducting a simulation, the result suggested that multi-hop topology may be an alternative green communication as it has better energy efficiency.
Energy efficiency analysis in WiFi communication with regard to video on demand transmission is investigated in~\cite{Trestian2012}. It utilized Android devices in a controlled IEEE 802.11g environment with five different quality levels of multimedia content. The results show that the quality of multimedia stream correlates with the energy efficiency. The results also show that TCP is more energy efficient than UDP in all situations.
...
% mobile network
A study regarding mobile data communication, such as GSM and UMTS with classic use of mobile phone, e.g., making a call and sending an SMS, is presented to compare the energy usage for particular tasks in 2G and 3G~\cite{Perrucci2009}.
Another low level analysis is also presented in~\cite{Vergara2012}. The energy footprint is characterized by performing real hardware level measurements on a modern broadband module.

% energy conserving
Several energy efficiency researches were also carried out, such as~\cite{Bandara2015}, that uses data driven approach to lessen the unneeded scanning or idle mode WiFi energy consumption.
A framework to conserve energy in wireless communication is also proposed in~\cite{Chareen2011}. This framework, namely WINA (Wireless Interface Notification and Activation), introduces a low-powered auxiliary receiver that receives signal from access point to activate relevant wireless interfaces on the mobile phone.
...
For Intro:
Currently, nearly half of the global population is benefiting from mobile communication~\cite{Fowdur2016}.
Main energy consuming part is wireless communication~\cite{Fowdur2016}.
WiFi is main means of communications~\cite{Fowdur2016}.
WiFi based positioning suffers from the interference in a coexistence environment~\cite{Pei2012}.
The mobile phone's batteries are not capable to keep up with the rapid growth of the other technologies, e.g., processing power and memory. This makes researches find a method to perform efficient tasks.
CPU and wireless communication are the largest consumers of energy sources in mobile devices~\cite{Crk2009}. 


For methodology:
Because wireless communication power footprint is not only affected by the size, but also the signal strength~\cite{Vergara2012}.
Main energy consuming part is wireless communication~\cite{Fowdur2016}
Network load and the signal quality level have a combined significant impact on the energy consumption~\cite{Trestian2012}
HTTP is chosen because it works on top of TCP, which is proven to be more energy efficient than UDP~\cite{Trestian2012}.
